...
Objects::Level::Level(const glm::vec2& pos, std::shared_ptr<Render::MasterRenderer> renderer)
  ...
  for(int i = 0; i < this->levelLayout.size(); i++) {
    for(int j = 0; j < this->levelLayout[i].size(); j++) {
      glm::vec2 pos = { j*16.0f, i*16.0f };
      switch(this->levelLayout[i][j]) {
        case 1:
          this->m_points10.push_back(std::shared_ptr<Points10>(new Points10(pos, renderer)));
          break;
        case 2:
          this->m_points50.push_back(std::shared_ptr<Points50>(new Points50(pos, renderer)));
          break;
        case 10 ... 69:
          Type type = (Type) this->levelLayout[i][j];
          this->m_walls.push_back(std::shared_ptr<Wall>(new Wall(type, pos, renderer)));
          break;
      }
    }
  }
